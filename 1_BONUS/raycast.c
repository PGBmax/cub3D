...
void    draw_floor_ceilling(t_game *game)
{
    ...
    {
        x = -1;
        float rayDirX0 = game->ray->dirX - game->ray->planeX;
        float rayDirY0 = game->ray->dirY - game->ray->planeY;
        float rayDirX1 = game->ray->dirX + game->ray->planeX;
        float rayDirY1 = game->ray->dirY + game->ray->planeY;
        ...
        float floorX = game->ray->posX + rowDistance * rayDirX0;
        float floorY = game->ray->posY + rowDistance * rayDirY0;
        while (++x < WIDTH)
        {
            int cellX = (int)floorX;
            int cellY = (int)floorY;

            int tx = (int)(S_BOX * (floorX - cellX)) & (S_BOX - 1);
            int ty = (int)(S_BOX * (floorY - cellY)) & (S_BOX - 1);
            
            floorX += floorStepX;
            floorY += floorStepY;
            uint32_t color;

            color = game->matrix->floor[tx][ty];
            mlx_put_pixel(game->screen, x, y, color);

            color = game->matrix->ceilling[tx][ty];
