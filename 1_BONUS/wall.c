...
/*   Updated: 2025/10/15 15:15:54 by pboucher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	choose_wall(t_game *game, int texX, int texY)
{
	...
	{
		color = game->matrix->door2[game->wall][texX][texY];
		return (color);
	}
	if (game->tab[game->ray->mapX][game->ray->mapY] == '3')
	{
		color = game->matrix->door2[game->wall][texX][texY] - 128;
		return (color);
	}
...
void	print_wall(t_game *game, int x)
{
	...
	while (++game->ray->drawStart <= game->ray->drawEnd)
	{
		...
		mlx_put_pixel(game->screen, x, game->ray->drawStart, (color + (game->h % 255  << 24) + (game->h % 255  << 16) + (game->h % 255 << 8)));
